...
/* Controller for Case type 1 (Unternehmensrecherche) */

class Case02 extends Controller {
    ...
    public function index() {
        ...
        $optional_views = array('application/views/case02/index.php');
    ...
    public function show($pk_cases) {
        ...
        $this->case_type = "case02";

        $this->sub_type_cases = $this->db_values->sub_type_cases;

        $this->judgement_table = $this->reports_model->getResultTableByCase($pk_cases);        

        $optional_views = array('application/views/case02/show.php', 'application/views/queries/index.php', 'application/views/urls/index.php');
        $this->loadViews($optional_views, true);
    }

    public function add() {

        $optional_views = array('application/views/case02/add.php');
        $this->loadViews($optional_views, true);
    }
    ...
    public function save() {
        ...
        header('Location: ' . URL . 'case02/show/' . $last_id . '?status=c_s');
    }
    ...
    public function update($pk_cases) {
        ...
        header('Location: ' . URL . 'case02/show/' . $pk_cases . '?status=c_u');
    }
    ...
    public function delete($pk_cases) {
        ...
        header('Location: ' . URL . 'case02' . '?status=c_d');
    }

    public function process_form($form_values) {
        $id_case02 = sanitze_string_for_db($form_values["id_case02"]);
        $type_cases = 2;
        $date_case02 = date("Y-m-d");
        $config_case02 = $form_values["select_sub_type"];
        $comment_case02 = sanitze_string_for_db($form_values["comment_case02"]);
        $config_json = process_json(open_external_file_to_read("/user/cases/" . $_SESSION["language_json"]["Language"] . "cases02_config.json"));
        $config_array = $config_json["Config"];
        $config_modules = "";
        $config_crawlers = "";
        ...
        $ip_case02 = $_SERVER['REMOTE_ADDR'];

        $form_values = array("id_cases" => $id_case02, "type_cases" => $type_cases, "sub_type_cases" => $config_case02, "fk_users" => $fk_users, "comment_cases" => $comment_case02, "date_cases" => $date_case02, "ip_cases" => $ip_case02, "config_json_cases" => $config);
