...
$js_template_url = URL."inc/js/aapvl_datatables_case01_template.js";

$php_template_url = URL."server/server_processing_case01_template.php";
...
$search_columns_template = '"targets": [1,2,3,4,5,6,7,8,9,12,13,14],';
...
$edit_columns_template = '"targets": [4,5,6,7,8,9,12,13,14],';
...
for($i = 0; $i < count($exp_php_template); $i++) {
  ...
  if (strpos($exp_php_template[$i], "'dt' => 16") !== false) {
    $split_line_php = $i+1;
  }
...
for($i = 0; $i < count($exp_write); $i++) {
  ...
  if (strpos($exp_write[$i], "$('td:eq(16)") !== false) {
    $split_line = $i+1;
  }
